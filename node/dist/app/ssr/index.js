import k from"fs";import{parseHTML as o}from"linkedom";import v from"os";import O from"../api/get_api_for_data_functions.js";import N from"../../lib/get_browser_safe_request.js";import S from"../settings/load.js";import q from"./set_base_attributes_in_html.js";import J from"./set_head_tags_in_html.js";import u from"../../lib/path_exists.js";import x from"../accounts/get_browser_safe_user.js";const{readFile:y}=k.promises,i=S(),h=process.env.NODE_ENV==="development",{document:C}=o("<div></div>"),E=(e={})=>{let t=e?.base_html;if(e?.mod_theme){const r=o(t);r.document.querySelector("body").setAttribute("data-mod-theme",e?.mod_theme),t=r.document.toString()}return t.replace("${css}",`
		...
		`).replace('<div id="app"></div>',`
			...
			<script type="application/json" id="__joystick_data__">
				${JSON.stringify(e?.data||{},(r,a)=>typeof a=="string"?a.replace(/</g,"\\u003c").replace(/-->/g,"--\\>").replace(/<\/script/gi,"<\\/script"):a)}
			</script>
			<script>
      ...
      ${h?'<script type="module" src="/_joystick/hmr/client.js"></script>':""}
		`)},I=(e={})=>e?.base_html.replace("${css}",`<style type="text/css">${e?.css}</style>`).replace("${subject}",e?.email_options?.subject).replace("${preheader}",e?.email_options?.preheader||"").replace('<div id="email"></div>',`<div id="email">${e?.html}</div>`),T=(e={})=>e?.is_email?I(e):E(e),A=async(e="")=>await u(`email/${e?`base_${e}`:"base"}.html`)?y(`email/${e?`base_${e}`:"base"}.html`,"utf-8"):(console.warn(`Could not find email/${e?`base_${e}`:"base"}.html`),""),B=async(e="")=>await u(`email/${e?`base_${e}`:"base"}.css`)?y(`email/${e?`base_${e}`:"base"}.css`,"utf-8"):(console.warn(`Could not find email/${e?`base_${e}`:"base"}.css`),""),D=(e=null,t={})=>e(t),F=async(e={})=>{const t=D(e?.component_to_render,e?.component_options),r=O(e?.req,e?.res,e?.api_schema),a=[],c=await t.render_for_ssr(r,e?.req,a,{linkedom_document:C,is_email:e?.is_email,is_dynamic_page_render:e?.is_dynamic_page_render});if(e?.is_dynamic_page_render)return c?.data;const p=e?.is_email?await B(e?.email_options?.base_html_name):null,$=e?.is_email?await A(e?.email_options?.base_html_name):null;let m="",f="";const l=e?.mod?.in_use&&e?.mod?.components_in_use&&e?.mod?.components_in_use?.length>0;if(l){const w=e?.mod?.theme==="light"?"dark":"light",s={...e?.mod?.css?.globals};delete s[w],m+=Object.values(s||{})?.reduce((_="",d="")=>(_+=d,_),"");const n=Object.entries(e?.mod?.css?.components)?.filter(([_]="")=>e?.mod?.components_in_use?.includes(_));for(let _=0;_<n?.length;_+=1){const[d,j]=n[_];m+=j[e?.mod?.theme]}}const g=T({is_email:e?.is_email,attributes:e?.attributes,base_html:e?.is_email?$:e?.base_html,component_instance:t,css:e?.is_email?`
			${p}
			${c?.css}
		`:c?.css,mod_version:e?.mod?.version,mod_in_use:e?.mod?.in_use,mod_tree_shaking:l,mod_css:m,mod_js:f,mod_theme:e?.mod?.theme,data:c?.data,email_options:e?.email_options,head:e?.head,html:c?.html,props:{theme:e?.mod?.theme,...e?.component_options?.props||{}},render_component_path:e?.render_component_path,render_layout_path:e?.render_layout_path,req:e?.req,translations:e?.component_options?.translations,url:e?.component_options?.url}),b=J(g,e?.head,e?.req);return q(b,e?.attributes)};var W=F;export{W as default};
