import v from"fs";import{parseHTML as d}from"linkedom";import O from"os";import S from"../api/get_api_for_data_functions.js";import N from"../../lib/get_browser_safe_request.js";import q from"../settings/load.js";import x from"./set_base_attributes_in_html.js";import J from"./set_head_tags_in_html.js";import o from"../../lib/path_exists.js";import A from"../accounts/get_browser_safe_user.js";import u from"../../lib/escape_html.js";import C from"./escape_ssr_data.js";const{readFile:p}=v.promises,c=q(),y=process.env.NODE_ENV==="development",{document:E}=d("<div></div>"),D=(e={})=>{let t=e?.base_html;if(e?.mod_theme){const r=d(t);r.document.querySelector("body").setAttribute("data-mod-theme",e?.mod_theme),t=r.document.toString()}return t.replace("${css}",`
      ...
      ${y?'<script type="module" src="/_joystick/hmr/client.js"></script>':""}
		`)},F=(e={})=>e?.base_html.replace("${css}",`<style type="text/css">${e?.css}</style>`).replace("${subject}",u(e?.email_options?.subject||"")).replace("${preheader}",u(e?.email_options?.preheader||"")).replace('<div id="email"></div>',`<div id="email">${e?.html}</div>`),I=(e={})=>e?.is_email?F(e):D(e),T=async(e="")=>{const t=`${e?`base_${e}`:"base"}.html`;return process._joystick_email_base_files?.[t]?process._joystick_email_base_files[t]:await o(`email/${t}`)?p(`email/${t}`,"utf-8"):(console.warn(`Could not find email/${t}`),"")},B=async(e="")=>{const t=`${e?`base_${e}`:"base"}.css`;return process._joystick_email_base_files?.[t]?process._joystick_email_base_files[t]:await o(`email/${t}`)?p(`email/${t}`,"utf-8"):(console.warn(`Could not find email/${t}`),"")},H=(e=null,t={})=>e(t),L=async(e={})=>{const t=H(e?.component_to_render,e?.component_options),r=S(e?.req,e?.res,e?.api_schema),h=[],_=await t.render_for_ssr(r,e?.req,h,{linkedom_document:E,is_email:e?.is_email,is_dynamic_page_render:e?.is_dynamic_page_render});if(e?.is_dynamic_page_render)return _?.data;const f=e?.is_email?await B(e?.email_options?.base_html_name):null,b=e?.is_email?await T(e?.email_options?.base_html_name):null;let s="",g="";const i=e?.mod?.in_use&&e?.mod?.components_in_use&&e?.mod?.components_in_use?.length>0;if(i){const k=e?.mod?.theme==="light"?"dark":"light",m={...e?.mod?.css?.globals};delete m[k],s+=Object.values(m||{})?.reduce((a="",n="")=>(a+=n,a),"");const l=Object.entries(e?.mod?.css?.components)?.filter(([a]="")=>e?.mod?.components_in_use?.includes(a));for(let a=0;a<l?.length;a+=1){const[n,w]=l[a];s+=w[e?.mod?.theme]}}const j=I({is_email:e?.is_email,attributes:e?.attributes,base_html:e?.is_email?b:e?.base_html,component_instance:t,css:e?.is_email?`
			${f}
			${_?.css}
		`:_?.css,mod_version:e?.mod?.version,mod_in_use:e?.mod?.in_use,mod_tree_shaking:i,mod_css:s,mod_js:g,mod_theme:e?.mod?.theme,data:C(_?.data),email_options:e?.email_options,head:e?.head,html:_?.html,props:{theme:e?.mod?.theme,...e?.component_options?.props||{}},render_component_path:e?.render_component_path,render_layout_path:e?.render_layout_path,req:e?.req,translations:e?.component_options?.translations,url:e?.component_options?.url}),$=J(j,e?.head,e?.req);return x($,e?.attributes)};var ee=L;export{ee as default};
