import b from"fs";import{parseHTML as g}from"linkedom";import j from"os";import k from"../api/get_api_for_data_functions.js";import v from"../../lib/get_browser_safe_request.js";import O from"../settings/load.js";import N from"./set_base_attributes_in_html.js";import S from"./set_head_tags_in_html.js";import o from"../../lib/path_exists.js";import"../../lib/tree_shake_mod_css.js";import J from"../accounts/get_browser_safe_user.js";const{readFile:i}=b.promises,a=O(),u=process.env.NODE_ENV==="development",{document:q}=g("<div></div>"),x=(e={})=>e?.base_html.replace("${css}",`
      ...
      ${u?'<script type="module" src="/_joystick/hmr/client.js"></script>':""}
		`),C=(e={})=>e?.base_html.replace("${css}",`<style type="text/css">${e?.css}</style>`).replace("${subject}",e?.email_options?.subject).replace("${preheader}",e?.email_options?.preheader||"").replace('<div id="email"></div>',`<div id="email">${e?.html}</div>`),E=(e={})=>e?.is_email?C(e):x(e),T=async(e="")=>await o(`email/${e?`base_${e}`:"base"}.html`)?i(`email/${e?`base_${e}`:"base"}.html`,"utf-8"):(console.warn(`Could not find email/${e?`base_${e}`:"base"}.html`),""),B=async(e="")=>await o(`email/${e?`base_${e}`:"base"}.css`)?i(`email/${e?`base_${e}`:"base"}.css`,"utf-8"):(console.warn(`Could not find email/${e?`base_${e}`:"base"}.css`),""),D=(e=null,t={})=>e(t),F=async(e={})=>{const t=D(e?.component_to_render,e?.component_options),p=k(e?.req,e?.api_schema),y=[],r=await t.render_for_ssr(p,e?.req,y,{linkedom_document:q,is_email:e?.is_email,is_dynamic_page_render:e?.is_dynamic_page_render});if(e?.is_dynamic_page_render)return r?.data;const f=e?.is_email?await B(e?.email_options?.base_html_name):null,h=e?.is_email?await T(e?.email_options?.base_html_name):null;let c="";const l=e?.mod?.theme,m=e?.mod?.plus?.css?.light||e?.mod?.plus?.css?.dark,d=e?.mod?.free?.css?.light||e?.mod?.free?.css?.dark;if((m||d)&&!t?.mod){const _=(e?.mod?.plus?.css||e?.mod?.free?.css)[l];c=await i(_,"utf-8")}if((m&&e?.mod?.plus?.map||d&&e?.mod?.free?.map)&&t?.mod){const _=e?.mod?.plus?.map||e?.mod?.free?.map,s=get_mod_css_from_map(_,t?.mod?.keep||[],l);c+=s}const $=E({is_email:e?.is_email,attributes:e?.attributes,base_html:e?.is_email?h:e?.base_html,component_instance:t,css:e?.is_email?`
			${f}
			${r?.css}
		`:r?.css,mod_css:c,data:Object.entries(r?.data||{})?.reduce((_={},[s,n])=>(_[s]=n?Buffer.from(JSON.stringify(n),"utf8").toString("base64"):"",_),{}),email_options:e?.email_options,head:e?.head,html:r?.html,props:e?.component_options?.props,render_component_path:e?.render_component_path,render_layout_path:e?.render_layout_path,req:e?.req,translations:e?.component_options?.translations,url:e?.component_options?.url}),w=S($,e?.head,e?.req);return N(w,e?.attributes)};var W=F;export{W as default};
