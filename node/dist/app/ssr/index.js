import j from"fs";import{parseHTML as o}from"linkedom";import k from"os";import v from"../api/get_api_for_data_functions.js";import O from"../../lib/get_browser_safe_request.js";import S from"../settings/load.js";import N from"./set_base_attributes_in_html.js";import q from"./set_head_tags_in_html.js";import u from"../../lib/path_exists.js";import J from"../accounts/get_browser_safe_user.js";const{readFile:y}=j.promises,d=S(),h=process.env.NODE_ENV==="development",{document:x}=o("<div></div>"),C=(e={})=>{let t=e?.base_html;if(e?.mod_theme){const c=o(t);c.document.querySelector("body").setAttribute("data-mod-theme",e?.mod_theme),t=c.document.toString()}return t.replace("${css}",`
      ...
      ${h?'<script type="module" src="/_joystick/hmr/client.js"></script>':""}
		`)},E=(e={})=>e?.base_html.replace("${css}",`<style type="text/css">${e?.css}</style>`).replace("${subject}",e?.email_options?.subject).replace("${preheader}",e?.email_options?.preheader||"").replace('<div id="email"></div>',`<div id="email">${e?.html}</div>`),T=(e={})=>e?.is_email?E(e):C(e),A=async(e="")=>await u(`email/${e?`base_${e}`:"base"}.html`)?y(`email/${e?`base_${e}`:"base"}.html`,"utf-8"):(console.warn(`Could not find email/${e?`base_${e}`:"base"}.html`),""),B=async(e="")=>await u(`email/${e?`base_${e}`:"base"}.css`)?y(`email/${e?`base_${e}`:"base"}.css`,"utf-8"):(console.warn(`Could not find email/${e?`base_${e}`:"base"}.css`),""),D=(e=null,t={})=>e(t),F=async(e={})=>{const t=D(e?.component_to_render,e?.component_options),c=v(e?.req,e?.res,e?.api_schema),p=[],r=await t.render_for_ssr(c,e?.req,p,{linkedom_document:x,is_email:e?.is_email,is_dynamic_page_render:e?.is_dynamic_page_render});if(e?.is_dynamic_page_render)return r?.data;const f=e?.is_email?await B(e?.email_options?.base_html_name):null,b=e?.is_email?await A(e?.email_options?.base_html_name):null;let i="",n="";if(e?.mod?.in_use&&!e?.mod?.components_in_use&&(i=e?.mod?.css[e?.mod?.theme]),e?.mod?.in_use&&(n=e?.mod?.js||""),e?.mod?.in_use&&e?.mod?.components_in_use&&e?.mod?.components_in_use?.length>0){const m=e?.mod?.theme==="light"?"dark":"light",l={...e?.mod?.css?.globals};delete l[m],i+=Object.values(l||{})?.reduce((_="",s="")=>(_+=s,_),"");const a=Object.entries(e?.mod?.css?.components)?.filter(([_]="")=>e?.mod?.components_in_use?.includes(_));for(let _=0;_<a?.length;_+=1){const[s,g]=a[_];i+=g[e?.mod?.theme]}}const $=T({is_email:e?.is_email,attributes:e?.attributes,base_html:e?.is_email?b:e?.base_html,component_instance:t,css:e?.is_email?`
			${f}
			${r?.css}
		`:r?.css,mod_css:i,mod_js:n||"",mod_theme:e?.mod?.theme,data:Object.entries(r?.data||{})?.reduce((m={},[l,a])=>(m[l]=a?Buffer.from(JSON.stringify(a),"utf8").toString("base64"):"",m),{}),email_options:e?.email_options,head:e?.head,html:r?.html,props:{theme:e?.mod?.theme,...e?.component_options?.props||{}},render_component_path:e?.render_component_path,render_layout_path:e?.render_layout_path,req:e?.req,translations:e?.component_options?.translations,url:e?.component_options?.url}),w=q($,e?.head,e?.req);return N(w,e?.attributes)};var U=F;export{U as default};
