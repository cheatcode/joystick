import O from"fs";import{parseHTML as o}from"linkedom";import S from"os";import N from"../api/get_api_for_data_functions.js";import q from"../../lib/get_browser_safe_request.js";import x from"../settings/load.js";import J from"./set_base_attributes_in_html.js";import A from"./set_head_tags_in_html.js";import u from"../../lib/path_exists.js";import C from"../accounts/get_browser_safe_user.js";import p from"../../lib/escape_html.js";import i from"./escape_ssr_data.js";const{readFile:y}=O.promises,c=x(),h=process.env.NODE_ENV==="development",{document:E}=o("<div></div>"),D=(e={})=>{let t=e?.base_html;if(e?.mod_theme){const r=o(t);r.document.querySelector("body").setAttribute("data-mod-theme",e?.mod_theme),t=r.document.toString()}return t.replace("${css}",`
        ...
        window.__joystick_page_url__ = ${e?.render_component_path?`"/_joystick/${e?.render_component_path}"`:null};
       	window.__joystick_request__ = ${JSON.stringify(i(q(e?.req)))};
        ...
        window.__joystick_user__ = ${JSON.stringify(i(C(e?.req?.context?.user)))};
			</script>
      ...
      ${h?'<script type="module" src="/_joystick/hmr/client.js"></script>':""}
		`)},F=(e={})=>e?.base_html.replace("${css}",`<style type="text/css">${e?.css}</style>`).replace("${subject}",p(e?.email_options?.subject||"")).replace("${preheader}",p(e?.email_options?.preheader||"")).replace('<div id="email"></div>',`<div id="email">${e?.html}</div>`),I=(e={})=>e?.is_email?F(e):D(e),T=async(e="")=>{const t=`${e?`base_${e}`:"base"}.html`;return process._joystick_email_base_files?.[t]?process._joystick_email_base_files[t]:await u(`email/${t}`)?y(`email/${t}`,"utf-8"):(console.warn(`Could not find email/${t}`),"")},B=async(e="")=>{const t=`${e?`base_${e}`:"base"}.css`;return process._joystick_email_base_files?.[t]?process._joystick_email_base_files[t]:await u(`email/${t}`)?y(`email/${t}`,"utf-8"):(console.warn(`Could not find email/${t}`),"")},H=(e=null,t={})=>e(t),L=async(e={})=>{const t=H(e?.component_to_render,e?.component_options),r=N(e?.req,e?.res,e?.api_schema),f=[],_=await t.render_for_ssr(r,e?.req,f,{linkedom_document:E,is_email:e?.is_email,is_dynamic_page_render:e?.is_dynamic_page_render});if(e?.is_dynamic_page_render)return _?.data;const b=e?.is_email?await B(e?.email_options?.base_html_name):null,g=e?.is_email?await T(e?.email_options?.base_html_name):null;let s="",j="";const m=e?.mod?.in_use&&e?.mod?.components_in_use&&e?.mod?.components_in_use?.length>0;if(m){const w=e?.mod?.theme==="light"?"dark":"light",l={...e?.mod?.css?.globals};delete l[w],s+=Object.values(l||{})?.reduce((a="",d="")=>(a+=d,a),"");const n=Object.entries(e?.mod?.css?.components)?.filter(([a]="")=>e?.mod?.components_in_use?.includes(a));for(let a=0;a<n?.length;a+=1){const[d,v]=n[a];s+=v[e?.mod?.theme]}}const $=I({is_email:e?.is_email,attributes:e?.attributes,base_html:e?.is_email?g:e?.base_html,component_instance:t,css:e?.is_email?`
			${b}
			${_?.css}
		`:_?.css,mod_version:e?.mod?.version,mod_in_use:e?.mod?.in_use,mod_tree_shaking:m,mod_css:s,mod_js:j,mod_theme:e?.mod?.theme,data:_?.data,email_options:e?.email_options,head:e?.head,html:_?.html,props:{theme:e?.mod?.theme,...e?.component_options?.props||{}},render_component_path:e?.render_component_path,render_layout_path:e?.render_layout_path,req:e?.req,translations:e?.component_options?.translations,url:e?.component_options?.url}),k=A($,e?.head,e?.req);return J(k,e?.attributes)};var ee=L;export{ee as default};
