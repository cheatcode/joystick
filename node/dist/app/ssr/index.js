import w from"fs";import{parseHTML as g}from"linkedom";import j from"os";import k from"../api/get_api_for_data_functions.js";import O from"../../lib/get_browser_safe_request.js";import v from"../settings/load.js";import N from"./set_base_attributes_in_html.js";import S from"./set_head_tags_in_html.js";import n from"../../lib/path_exists.js";import"../../lib/tree_shake_mod_css.js";import J from"../accounts/get_browser_safe_user.js";const{readFile:o}=w.promises,a=v(),u=process.env.NODE_ENV==="development",{document:q}=g("<div></div>"),x=(e={})=>e?.base_html.replace("${css}",`
      ...
      ${u?'<script type="module" src="/_joystick/hmr/client.js"></script>':""}
		`),E=(e={})=>e?.base_html.replace("${css}",`<style type="text/css">${e?.css}</style>`).replace("${subject}",e?.email_options?.subject).replace("${preheader}",e?.email_options?.preheader||"").replace('<div id="email"></div>',`<div id="email">${e?.html}</div>`),T=(e={})=>e?.is_email?E(e):x(e),C=async(e="")=>await n(`email/${e?`base_${e}`:"base"}.html`)?o(`email/${e?`base_${e}`:"base"}.html`,"utf-8"):(console.warn(`Could not find email/${e?`base_${e}`:"base"}.html`),""),H=async(e="")=>await n(`email/${e?`base_${e}`:"base"}.css`)?o(`email/${e?`base_${e}`:"base"}.css`,"utf-8"):(console.warn(`Could not find email/${e?`base_${e}`:"base"}.css`),""),R=(e=null,t={})=>e(t),B=async(e={})=>{const t=R(e?.component_to_render,e?.component_options),p=k(e?.req,e?.api_schema),y=[],_=await t.render_for_ssr(p,e?.req,y,{linkedom_document:q,is_email:e?.is_email,is_dynamic_page_render:e?.is_dynamic_page_render});if(e?.is_dynamic_page_render)return _?.data;const f=e?.is_email?await H(e?.email_options?.base_html_name):null,h=e?.is_email?await C(e?.email_options?.base_html_name):null;let c="";const i=e?.mod?.theme,l=e?.mod?.plus?.css?.light||e?.mod?.plus?.css?.dark,m=e?.mod?.free?.css?.light||e?.mod?.free?.css?.dark;if((l||m)&&e?.mod?.keep?.length===0&&(c=(e?.mod?.plus?.css||e?.mod?.free?.css)[i]),(l&&e?.mod?.plus?.map||m&&e?.mod?.free?.map)&&e?.mod?.keep?.length>0){const r=e?.mod?.plus?.map||e?.mod?.free?.map,s=get_mod_css_from_map(r,t?.mod?.keep||[],i);c+=s}console.log("GOT TO HERE");const $=T({is_email:e?.is_email,attributes:e?.attributes,base_html:e?.is_email?h:e?.base_html,component_instance:t,css:e?.is_email?`
			${f}
			${_?.css}
		`:_?.css,mod_css:c,data:Object.entries(_?.data||{})?.reduce((r={},[s,d])=>(r[s]=d?Buffer.from(JSON.stringify(d),"utf8").toString("base64"):"",r),{}),email_options:e?.email_options,head:e?.head,html:_?.html,props:e?.component_options?.props,render_component_path:e?.render_component_path,render_layout_path:e?.render_layout_path,req:e?.req,translations:e?.component_options?.translations,url:e?.component_options?.url}),b=S($,e?.head,e?.req);return N(b,e?.attributes)};var W=B;export{W as default};
