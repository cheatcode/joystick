import w from"fs";import{parseHTML as d}from"linkedom";import j from"os";import k from"../api/get_api_for_data_functions.js";import v from"../../lib/get_browser_safe_request.js";import O from"../settings/load.js";import S from"./set_base_attributes_in_html.js";import N from"./set_head_tags_in_html.js";import o from"../../lib/path_exists.js";import q from"../accounts/get_browser_safe_user.js";const{readFile:u}=w.promises,n=O(),y=process.env.NODE_ENV==="development",{document:J}=d("<div></div>"),x=(e={})=>{let t=e?.base_html;if(e?.mod_theme){const i=d(t);i.document.querySelector("body").setAttribute("data-mod-theme",e?.mod_theme),t=i.document.toString()}return t.replace("${css}",`
      ...
      ${y?'<script type="module" src="/_joystick/hmr/client.js"></script>':""}
		`)},C=(e={})=>e?.base_html.replace("${css}",`<style type="text/css">${e?.css}</style>`).replace("${subject}",e?.email_options?.subject).replace("${preheader}",e?.email_options?.preheader||"").replace('<div id="email"></div>',`<div id="email">${e?.html}</div>`),E=(e={})=>e?.is_email?C(e):x(e),T=async(e="")=>await o(`email/${e?`base_${e}`:"base"}.html`)?u(`email/${e?`base_${e}`:"base"}.html`,"utf-8"):(console.warn(`Could not find email/${e?`base_${e}`:"base"}.html`),""),A=async(e="")=>await o(`email/${e?`base_${e}`:"base"}.css`)?u(`email/${e?`base_${e}`:"base"}.css`,"utf-8"):(console.warn(`Could not find email/${e?`base_${e}`:"base"}.css`),""),B=(e=null,t={})=>e(t),D=async(e={})=>{const t=B(e?.component_to_render,e?.component_options),i=k(e?.req,e?.api_schema),p=[],r=await t.render_for_ssr(i,e?.req,p,{linkedom_document:J,is_email:e?.is_email,is_dynamic_page_render:e?.is_dynamic_page_render});if(e?.is_dynamic_page_render)return r?.data;const h=e?.is_email?await A(e?.email_options?.base_html_name):null,f=e?.is_email?await T(e?.email_options?.base_html_name):null;let c="";if(e?.mod?.in_use&&!e?.mod?.components_in_use&&(c=e?.mod?.css[e?.mod?.theme]),e?.mod?.in_use&&e?.mod?.components_in_use&&e?.mod?.components_in_use?.length>0){const l=e?.mod?.theme==="light"?"dark":"light",m={...e?.mod?.css?.globals};delete m[l],c+=Object.values(m||{})?.reduce((_="",s="")=>(_+=s,_),"");const a=Object.entries(e?.mod?.css?.components)?.filter(([_]="")=>e?.mod?.components_in_use?.includes(_));for(let _=0;_<a?.length;_+=1){const[s,g]=a[_];c+=g[e?.mod?.theme]}}const b=E({is_email:e?.is_email,attributes:e?.attributes,base_html:e?.is_email?f:e?.base_html,component_instance:t,css:e?.is_email?`
			${h}
			${r?.css}
		`:r?.css,mod_css:c,mod_theme:e?.mod?.theme,data:Object.entries(r?.data||{})?.reduce((l={},[m,a])=>(l[m]=a?Buffer.from(JSON.stringify(a),"utf8").toString("base64"):"",l),{}),email_options:e?.email_options,head:e?.head,html:r?.html,props:e?.component_options?.props,render_component_path:e?.render_component_path,render_layout_path:e?.render_layout_path,req:e?.req,translations:e?.component_options?.translations,url:e?.component_options?.url}),$=N(b,e?.head,e?.req);return S($,e?.attributes)};var Q=D;export{Q as default};
