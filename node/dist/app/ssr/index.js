import k from"fs";import{parseHTML as d}from"linkedom";import v from"os";import O from"../api/get_api_for_data_functions.js";import N from"../../lib/get_browser_safe_request.js";import S from"../settings/load.js";import q from"./set_base_attributes_in_html.js";import J from"./set_head_tags_in_html.js";import o from"../../lib/path_exists.js";import x from"../accounts/get_browser_safe_user.js";const{readFile:u}=k.promises,c=S(),h=process.env.NODE_ENV==="development",{document:C}=d("<div></div>"),E=(e={})=>{let t=e?.base_html;if(console.log("DATA in bhrfb",e?.data),e?.mod_theme){const a=d(t);a.document.querySelector("body").setAttribute("data-mod-theme",e?.mod_theme),t=a.document.toString()}return t.replace("${css}",`
      ...
      ${h?'<script type="module" src="/_joystick/hmr/client.js"></script>':""}
		`)},A=(e={})=>e?.base_html.replace("${css}",`<style type="text/css">${e?.css}</style>`).replace("${subject}",e?.email_options?.subject).replace("${preheader}",e?.email_options?.preheader||"").replace('<div id="email"></div>',`<div id="email">${e?.html}</div>`),T=(e={})=>e?.is_email?A(e):E(e),D=async(e="")=>await o(`email/${e?`base_${e}`:"base"}.html`)?u(`email/${e?`base_${e}`:"base"}.html`,"utf-8"):(console.warn(`Could not find email/${e?`base_${e}`:"base"}.html`),""),I=async(e="")=>await o(`email/${e?`base_${e}`:"base"}.css`)?u(`email/${e?`base_${e}`:"base"}.css`,"utf-8"):(console.warn(`Could not find email/${e?`base_${e}`:"base"}.css`),""),B=(e=null,t={})=>e(t),F=async(e={})=>{console.log({component_to_render:e?.component_to_render,component_options:e?.component_options});const t=B(e?.component_to_render,e?.component_options),a=O(e?.req,e?.res,e?.api_schema),p=[],r=await t.render_for_ssr(a,e?.req,p,{linkedom_document:C,is_email:e?.is_email,is_dynamic_page_render:e?.is_dynamic_page_render});if(console.log({ssr_render:r}),e?.is_dynamic_page_render)return r?.data;const y=e?.is_email?await I(e?.email_options?.base_html_name):null,g=e?.is_email?await D(e?.email_options?.base_html_name):null;let i="",$="";const l=e?.mod?.in_use&&e?.mod?.components_in_use&&e?.mod?.components_in_use?.length>0;if(l){const w=e?.mod?.theme==="light"?"dark":"light",m={...e?.mod?.css?.globals};delete m[w],i+=Object.values(m||{})?.reduce((_="",s="")=>(_+=s,_),"");const n=Object.entries(e?.mod?.css?.components)?.filter(([_]="")=>e?.mod?.components_in_use?.includes(_));for(let _=0;_<n?.length;_+=1){const[s,j]=n[_];i+=j[e?.mod?.theme]}}const b=T({is_email:e?.is_email,attributes:e?.attributes,base_html:e?.is_email?g:e?.base_html,component_instance:t,css:e?.is_email?`
			${y}
			${r?.css}
		`:r?.css,mod_version:e?.mod?.version,mod_in_use:e?.mod?.in_use,mod_tree_shaking:l,mod_css:i,mod_js:$,mod_theme:e?.mod?.theme,data:r?.data,email_options:e?.email_options,head:e?.head,html:r?.html,props:{theme:e?.mod?.theme,...e?.component_options?.props||{}},render_component_path:e?.render_component_path,render_layout_path:e?.render_layout_path,req:e?.req,translations:e?.component_options?.translations,url:e?.component_options?.url}),f=J(b,e?.head,e?.req);return q(f,e?.attributes)};var W=F;export{W as default};
