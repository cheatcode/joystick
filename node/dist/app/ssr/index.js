import f from"fs";import{parseHTML as $}from"linkedom";import h from"os";import b from"../api/get_api_for_data_functions.js";import w from"../../lib/get_browser_safe_request.js";import j from"../settings/load.js";import g from"./set_base_attributes_in_html.js";import k from"./set_head_tags_in_html.js";import s from"../../lib/path_exists.js";import v from"../accounts/get_browser_safe_user.js";const{readFile:c}=f.promises,_=j(),l=process.env.NODE_ENV==="development",{document:O}=$("<div></div>"),N=(e={})=>e?.base_html.replace("${css}",`<style type="text/css" js-css>${e?.css}</style>`).replace('<div id="app"></div>',`
      ...
      ${l?'<script type="module" src="/_joystick/hmr/client.js"></script>':""}
		`),S=(e={})=>e?.base_html.replace("${css}",`<style type="text/css">${e?.css}</style>`).replace("${subject}",e?.email_options?.subject).replace("${preheader}",e?.email_options?.preheader||"").replace('<div id="email"></div>',`<div id="email">${e?.html}</div>`),J=(e={})=>e?.is_email?S(e):N(e),q=async(e="")=>await s(`email/${e?`base_${e}`:"base"}.html`)?c(`email/${e?`base_${e}`:"base"}.html`,"utf-8"):(console.warn(`Could not find email/${e?`base_${e}`:"base"}.html`),""),x=async(e="")=>await s(`email/${e?`base_${e}`:"base"}.css`)?c(`email/${e?`base_${e}`:"base"}.css`,"utf-8"):(console.warn(`Could not find email/${e?`base_${e}`:"base"}.css`),""),C=(e=null,t={})=>e(t),E=async(e={})=>{const t=C(e?.component_to_render,e?.component_options),n=b(e?.req,e?.api_schema),o=[],r=await t.render_for_ssr(n,e?.req,o,{linkedom_document:O,is_email:e?.is_email}),m=e?.is_email?await x(e?.email_options?.base_html_name):null,d=e?.is_email?await q(e?.email_options?.base_html_name):null,u=J({is_email:e?.is_email,attributes:e?.attributes,base_html:e?.is_email?d:e?.base_html,component_instance:t,css:e?.is_email?`
			${m}
			${r?.css}
		`:r?.css,data:Object.entries(r?.data||{})?.reduce((i={},[y,a])=>(i[y]=a?Buffer.from(JSON.stringify(a),"utf8").toString("base64"):"",i),{}),email_options:e?.email_options,head:e?.head,html:r?.html,props:e?.component_options?.props,render_component_path:e?.render_component_path,render_layout_path:e?.render_layout_path,req:e?.req,translations:e?.component_options?.translations,url:e?.component_options?.url}),p=k(u,e?.head,e?.req);return g(p,e?.attributes)};var z=E;export{z as default};
