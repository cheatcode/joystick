export default (listeners = {}) => {
  ...
  process.on("multipleResolves", async (type, promise, value) => {
    if (
      listeners?.multipleResolves &&
      typeof listeners.multipleResolves === "function"
    ) {
  ...
  process.on("rejectionHandled", async (promise) => {
    if (
      listeners?.rejectionHandled &&
      typeof listeners.rejectionHandled === "function"
    ) {
  ...
  process.on("uncaughtException", async (error, origin) => {
    ...
    if (
      ...
      typeof listeners.uncaughtException === "function"
    ) {
  ...
  process.on("uncaughtExceptionMonitor", async (error) => {
    if (
      ...
      typeof listeners.uncaughtExceptionMonitor === "function"
    ) {
  ...
  process.on("unhandledRejection", async (error) => {
    ...
    if (
      ...
      typeof listeners.unhandledRejection === "function"
    ) {
