...
export class App {
  ...
  async invalidateCache() {
    const uiFiles = fs.existsSync(`${getBuildPath()}ui`)
      ? await readDirectory(`${getBuildPath()}ui`)
      : [];
    const cacheFiles = uiFiles?.filter((filePath) =>
      filePath?.includes("_cache")
    );
  ...
  initDeploy() {
    if (process.env.NODE_ENV === "production" && process.env.IS_PUSH_DEPLOYED) {
      this.express.app.get("/api/_push/pre-version", async (req, res) => {
        ...
        if (
          req?.headers["x-instance-token"] === instanceToken?.replace("\n", "")
        ) {
          if (
            this.options?.events?.onBeforeDeployment &&
            typeof this.options?.events?.onBeforeDeployment === "function"
          ) {
        ...
        return res
          .status(403)
          .send(
            "Sorry, you must pass a valid instance token to access this endpoint."
          );
      });
      ...
      this.express.app.get("/api/_push/logs", async (req, res) => {
        ...
        if (
          ...
          const logs = execSync(
            `export NODE_ENV=production && instance logs${
      ...
      this.express.app.get("/api/_push/metrics", async (req, res) => {
        ...
        if (
          ...
          const metrics = execSync(
            `export NODE_ENV=production && instance metrics`
          );
  ...
  initRoutes(routes = {}) {
    Object.entries(routes).forEach(([path, callback]) => {
      ...
      const isFunctionBasedRoute =
        path && callback && typeof callback === "function";
      ...
      const invalidMethods = isObjectBasedRoute
        ? methodsForRoute.filter(
            (method) => !supportedHTTPMethods.includes(method)
          )
        : [];
      const isValidMethod =
        Array.isArray(methodsForRoute) && invalidMethods.length === 0;
      const isValidHandler =
        (isFunctionBasedRoute && typeof callback === "function") ||
        (isObjectBasedRoute &&
          callback &&
          callback.handler &&
      ...
      if (isFunctionBasedRoute && !isValidHandler) {
        log(
          `Cannot register route ${path}. When defining a route using the function-based pattern, route must be set to a function.`,
          {
            level: "danger",
      ...
      if (isObjectBasedRoute && !isValidHandler) {
        log(
          `Cannot register route ${path}. When defining a route using the object-based pattern, the handler property must be set to a function.`,
          {
            level: "danger",
      ...
      if (isObjectBasedRoute && invalidMethods.length > 0) {
        log(
          `Cannot register route ${path}. When defining a route using the object-based pattern, you can only set method (single HTTP method as a string) or methods (array of HTTP methods as strings), not both.`,
          {
            level: "danger",
            docs: "https://github.com/cheatcode/joystick#defining-routes-for-specific-http-methods",
          }
      ...
      if (isObjectBasedRoute && !method && !methods) {
        log(
          `Cannot register route ${path}. When defining a route using the object-based pattern, you must pass a method (single HTTP method as a string) or methods (array of HTTP methods as strings) for the route.`,
          {
            level: "danger",
            docs: "https://github.com/cheatcode/joystick#defining-routes-for-specific-http-methods",
          }
      ...
      if (isObjectBasedRoute && method && !isValidMethod) {
        log(
          `Cannot register route ${path}. When defining a route using the object-based pattern, the method property must be set to a valid HTTP method: ${supportedHTTPMethods.join(
            ", "
          )}.`,
          {
            level: "danger",
      ...
      if (isObjectBasedRoute && methods && !isValidMethod) {
        log(
          `Cannot register route ${path}. When defining a route using the object-based pattern, the methods property must be set to an array of valid HTTP methods: ${supportedHTTPMethods.join(
            ", "
          )}.`,
          {
            level: "danger",
      ...
      if (
        ...
        methodsForRoute.forEach((method) => {
          this.express.app[method](
            path,
            ...[
              ...(Array.isArray(callback?.middleware)
                ? callback?.middleware
                : []),
              async (req, res, next) => {
                callback.handler(
  ...
  initWebsockets(userWebsockets = {}) {
        ...
        websocketDefinition.server.on(
          ...
          function connection(websocketConnection, connectionRequest) {
            try {
              ...
              const emitterId = connectionParams?.id
                ? `${websocketName}_${connectionParams?.id}`
                : websocketName;
    ...
    this.express.server.on("upgrade", (request, socket, head) => {
      ...
      const websocketName = (request?.url
        ?.replace("/api/_websockets/", "")
        .split("?") || [])[0];
  ...
  initAccounts() {
    this.express.app.get("/api/_accounts/authenticated", async (req, res) => {
      ...
      return res
        .status(status)
        .send(JSON.stringify({ status, authenticated: !loginTokenHasExpired }));
    });
    ...
    this.express.app.get("/api/_accounts/user", async (req, res) => {
      ...
      const status = !loginTokenHasExpired ? 200 : 401;
      const user = getOutput(
        req?.context?.user,
        req?.body?.output || defaultUserOutputFields
      );
  ...
  initUploaders(uploaders = {}) {
    ...
    Object.entries(uploaders).forEach(([uploaderName, uploaderOptions]) => {
      ...
      if (errors?.length === 0) {
        ...
        app.post(
          `/api/_uploaders/${formattedUploaderName}`,
          (req, res, next) => {
            let progress = 0;
            ...
            const providers = uploaderOptions?.providers?.includes("local")
              ? uploaderOptions?.providers.length
              : uploaderOptions?.providers?.length + 1;
            const totalFileSizeAllProviders = fileSize * providers;

            req.on("data", (chunk) => {
              progress += chunk.length;
              const percentage = Math.round(
                (progress / totalFileSizeAllProviders) * 100
              );
          ...
          multerMiddleware,
          async (req, res) => {
            const input = req?.headers["x-joystick-upload-input"]
              ? JSON.parse(req?.headers["x-joystick-upload-input"])
              : {};
                  ...
                  JSON.stringify({
                    errors: (errors || []).map((error) => {
                      return formatAPIError(
                        new Error(error?.message, "validation")
                      );
                    }),
                  })
                );
  ...
  initCronJobs(cronJobs = null) {
    if (cronJobs && typeof cronJobs === "object" && !Array.isArray(cronJobs)) {
      ...
      for (let i = 0; i < cronJobDefinitions.length; i += 1) {
        ...
        if (
          cronJobOptions?.schedule &&
          cronJobOptions?.run &&
          typeof cronJobOptions?.run === "function"
        ) {
          process.cron = {
            ...(process.queues || {}),
            [cronJobName]: cron.schedule(cronJobOptions?.schedule, () => {
              if (
                cronJobOptions?.logAtRunTime &&
                typeof cronJobOptions?.logAtRunTime === "string"
              ) {
