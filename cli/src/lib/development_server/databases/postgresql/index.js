  ...
  try {
    ...
    const database_process = process.platform === 'linux' ? child_process.spawn(
      ...
      [
        ...
        'bash',
        '-c',
        `PGDATA="${get_platform_safe_path(`${process.cwd()}/.joystick/data/postgresql_${port}`)}" ./postgres -p ${postgresql_port} > /dev/null 2>&1`,
      ],
      ...
      // ],
      { cwd: joystick_postgresql_bin_path, shell: '/bin/bash', stdio: ['ignore', 'pipe', 'pipe'] }
    ) : child_process.spawn(
