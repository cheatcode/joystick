    ...
    setup(build) {
      build.onLoad({ filter: /\.js$/ }, (args) => {
        ...
        if (shouldBootstrap) {
          ...
          if (!match) {
            console.log(" ");
            console.warn(chalk.yellowBright(`All components in the ui/layouts directory must have an export default statement (e.g., export default MyLayout). Please check the file at ${args.path}.`));
            console.log(" ");
            return;
          }
          ...
          if (componentName) {
            ...
            return {
              contents: code2.replace(`${match};`, `if (
    ...
    setup(build) {
      build.onLoad({ filter: /\.js$/ }, (args) => {
        ...
        if (shouldBootstrap) {
          ...
          if (!match) {
            console.log(" ");
            console.warn(chalk.yellowBright(`All components in the ui/pages directory must have an export default statement (e.g., export default MyLayout). Please check the file at ${args.path}.`));
            console.log(" ");
            return;
          }
          const matchParts = match && match.split(" ") || [];
