...
const set = (setter_name = "", setter_options = {}) => {
  if (typeof window.fetch !== 'undefined') {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (setter_options?.user) {
        headers.Cookie = generate_cookie_header({
          joystick_login_token: setter_options?.user?.joystick_login_token,
          joystick_login_token_expires_at: setter_options?.user?.joystick_login_token_expires_at,
        });
      }
