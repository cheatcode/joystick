import h from"pg";import n from"chalk";import g from"os";import m from"fs";import l from"../sql.js";const{Pool:d}=h,p=async(c={},u=2610)=>{const s=c?.connection||{hosts:[{hostname:"127.0.0.1",port:u}],database:"app",username:(g.userInfo()||{}).username||"",password:""};try{const a=s.hosts&&s.hosts[0],i={user:s?.username||"",database:s?.database,password:s?.password||"",host:a?.hostname,port:a?.port,...c?.options||{}};c?.options?.ssl?.ca&&(i.ssl={ca:m.readFileSync(c?.options?.ssl?.ca)});const r=new d(i);return Promise.resolve({pool:r,query:(...t)=>r.query(...t).then(e=>e?.rows||[]).catch(e=>{throw console.log(n.redBright(`
