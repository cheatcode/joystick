var p=(t="",e={})=>{throw new Error(`[joystick${t?`.${t}`:""}] ${e.message||e.reason||e}`)};var x=t=>{try{return typeof t=="function"}catch(e){p("types.isFunction",e)}};var c=t=>{try{return!!(t&&typeof t=="object"&&!Array.isArray(t))}catch(e){p("types.isObject",e)}};var u=t=>{try{return typeof t=="string"}catch(e){p("types.isString",e)}};var d=(t="",e={})=>{try{let i="";if(t&&u(t))return i=t,i;if(t&&x(t))return i=t(e),i;if(t&&c(t)){let s=t?.print&&(u(t?.print)||x(t?.print)),a=t?.min&&c(t?.min),m=t?.min?.width&&c(t?.min?.width),y=t?.min?.height&&c(t?.min?.height),l=t?.max&&c(t?.max),g=t?.max?.width&&c(t?.max?.width),w=t?.max?.height&&c(t?.max?.height);if(s&&(i+=`
          @media print {
